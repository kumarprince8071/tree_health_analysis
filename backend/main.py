...
async def chat_with_model(query: ChatQuery):
    try:
        ...
        # Handle queries about total area
            ...
            response = f"The total area covered by the detected trees is {total_area:.2f} square units."
        ...
        # Handle queries about average tree area
            ...
            average_tree_area = query.average_tree_area
        ...
        # Handle queries about image metadata
            ...
            metadata = query.image_metadata
            response = f"Image Metadata:\nWidth: {metadata.get('width')} pixels\nHeight: {metadata.get('height')} pixels\nCRS: {metadata.get('crs')}\nTransform: {metadata.get('transform')}"
        ...
        # Handle other queries using the Mistral AI API
            ...
            if not api_key:
                ...
                chat_context = f"""
                You are an AI assistant that provides information about satellite imagery analysis. 
                The image has {query.tree_count} trees detected, covering a total area of {query.total_area:.2f} square units. 
                The average area per tree is {query.average_tree_area:.2f} square units.
                """
                messages = [
...
# # Endpoint to serve GeoJSON files
...
#         overlap = 50  # Adjusted overlap
#         logger.info(f"Using tile size: {tile_size}, overlap: {overlap}")
...
#             "crs": crs.to_string(),
#             "transform": transform_raster.to_gdal()
#         }
...
#     except Exception as e:
#         logger.error(f"Error processing model outputs: {e}")
#         return JSONResponse(status_code=500, content={"message": f"Failed to process model outputs: {str(e)}"})
...
#     except Exception as e:
#         logger.error(f"Error in /chat endpoint: {e}")
#         return JSONResponse(status_code=500, content={"message": f"An error occurred during chat processing: {str(e)}"})

# # Run the application
# if __name__ == "__main__":
#     uvicorn.run("main:app", host="0.0.0.0", port=8000, timeout_keep_alive=600)
